...
const mockQuiz = {
  quiz: [
    ...
    },
    {
      category: "movies",
      question:
        "Which actor played the lead role in the movie 'The Shawshank Redemption'?",
...
io.on("connection", (socket) => {
  ...
  socket.on("quizFinished", async (data) => {
    ...
    const text = `${name} är klar med quizzen med en genomsnittlig svarstid på ${
      Math.round(result.averageUserResponseTime * 10) / 10
    }s och skrapade ihop ${result.correctAnswers} ${
      result.correctAnswers === 1 ? "rätt" : "rätta"
    } svar. ${responseEndPhrase[result.correctAnswers]}`;
    global.io.to(room).emit("messageResponse", { ...data, text });

    if (
      finishedQuizzesResults.length ===
      users.filter((user) => user.room === room).length
    ) {
      const quizResults = finishedQuizzesResults.filter(
        (result) => result.room === room
      );
  ...
  socket.on("generateImage", async (prompt) => {
    global.io.to(socket.id).emit("generateImageInitiated", prompt);
    ...
    worker.on("message", (response) => {
      ...
      global.io.to(socket.id).emit("generateImageResponse", response);
    });

    worker.on("error", (err) => {
      console.error(`Worker error: ${err}`);
      global.io.to(socket.id).emit("error", error);
    });
  });
});
